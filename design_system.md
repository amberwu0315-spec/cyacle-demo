## 01. 👀 设计变量数据库 (Design Tokens)

> **核心指令 (Core Instruction):**
> 以下是本项目的**唯一真理来源 (Single Source of Truth)**。
> - 在生成 Tailwind 代码时，请优先使用与其最接近的标准类名（如 `p-2` 对应 `8px`）。
> - 如果没有完美匹配的 Tailwind 预设，请使用**Arbitrary Values** (例如 `bg-[#F5F6F8]` 或 `rounded-[2px]`) 来严格对齐视觉。

| 分类 (Category) | 变量名 (Token Key) | 具体数值 (Value) | 备注 (Usage Note) |
| :--- | :--- | :--- | :--- |
| **基础色** | `Canvas-BG` | `#F5F6F8` | 用于复杂绘图区域、仪表盘、弹窗内容区的底色（严格统一） |
| **灰阶** | `Text-Secondary` | `#6B7280` | 次要描述、占位符文字（基于标准 SaaS 灰阶推导） |
| **灰阶** | `Text-Disabled` | `#9CA3AF` | 禁用状态的文字颜色 |
| **灰阶** | `Border-Hover` | `#4D4D4D` | 边框悬停状态颜色（同步标题主色提升对比度） |
| **语义色** | `Success` | `#3BAA6D` | 操作成功、操作完成的状态 |
| **语义色** | `Warning` | `#E7A23C` | 待核实、已手动调整状态 |
| **语义色** | `Error` | `#E35B5B` | 操作失败、数据异常、对比差异巨大提示 |
| **语义色** | `Info` | `#4CA6D1` | 系统提示、引导信息颜色 |
| **间距** | `Spacing-XS` | `4px` | 极小间距（同步标准圆角基数） |
| **间距** | `Spacing-SM` | `8px` | 核心间距基数 |
| **间距** | `Spacing-LG` | `16px` | 模块间的结构间距（8px 的两倍） |
| **圆角** | `Radius-SM` | `2px` | 极小组件（如状态标签 Tag） |
| **圆角** | `Radius-LG` | `8px` | 大容器（如全局弹窗，同步标准间距基数） |
| **圆角** | `Radius-XL` | `12px` | 仅用于工作台大容器 (Workbench)
| **阴影** | `Shadow-SM` | `0 1px 2px rgba(0,0,0,0.05)` | 静态卡片的极轻微描边感 |
| **阴影** | `Shadow-MD` | `0 4px 6px rgba(0,0,0,0.1)` | 悬浮、菜单弹出时的阴影 |
| **层级** | `Z-Index-Nav` | `1000` | 导航栏的垂直层级 |
| **层级** | `Z-Index-Modal` | `2000` | 弹窗、遮罩层的垂直层级 |
...
*   **Template A**: L2 Sidebar + Content.
*   **Template B**: L2 + Header + Content + Footer.
*   **Template C**: L2 + L3 + Header + Content + Footer.
...
> **核心原则 (Core Principle):**
> 所有布局遵循 **12px 间距规则** (`gap-3`)，内容区外边距统一为 `p-3`。

> **⚠️ 重要提示 (Important Note):**
> 以下"适用场景"仅为**典型示例**，不是硬性规定。实际开发中应根据具体业务需求和内容特点**灵活选择**布局类型。布局的选择应基于信息层级、交互关系和用户体验，而非机械套用场景描述。

---

##### Layout-A: 左右非对称布局 (Asymmetric Split)
**结构**: 左侧配置区 + 右侧结果区 (4:8 比例)
...
<div class="flex-1 bg-[#F5F6F8] p-3">
  ...
  <div class="grid grid-cols-3 gap-3 h-full">
    <!-- 左侧配置区 (1 列) -->
    <div class="col-span-1 bg-white rounded-md p-6 overflow-y-auto" data-type="detail">
      <h3 class="text-sm font-medium text-gray-700 mb-4">配置面板</h3>
      <!-- 表单控件... -->
    </div>
    
    <!-- 右侧结果区 (2 列) -->
    <div class="col-span-2 bg-white rounded-md p-6 overflow-y-auto" data-type="detail">
      <h3 class="text-sm font-medium text-gray-700 mb-4">结果展示</h3>
      <!-- 数据内容... -->
    </div>
  </div>
</div>
...
##### Layout-B: 垂直堆叠布局 (Vertical Stack)
**结构**: 单列垂直排列
...
```html
<!-- 内容区容器 -->
<div class="flex-1 bg-[#F5F6F8] p-3 overflow-y-auto">
  <!-- Flex 垂直布局 -->
  <div class="flex flex-col gap-3">
    <!-- 模块 1 -->
    <div class="bg-white rounded-md p-6" data-type="detail">
    ...
    <!-- 模块 2 -->
    <div class="bg-white rounded-md p-6" data-type="detail">
      <h3 class="text-sm font-medium text-gray-700 mb-4">扩展属性</h3>
      <!-- 内容... -->
    </div>
    
    <!-- 模块 3 -->
    <div class="bg-white rounded-md p-6" data-type="detail">
      <h3 class="text-sm font-medium text-gray-700 mb-4">关联数据</h3>
      <!-- 内容... -->
    </div>
  </div>
</div>
...
##### Layout-C: 顶栏+画布布局 (Toolbar + Canvas)
**结构**: 顶部工具栏 + 下方内容区
...
<div class="flex-1 bg-[#F5F6F8] p-3">
  ...
  <div class="flex flex-col gap-3 h-full">
    <!-- 顶部工具栏 -->
    <div class="bg-white rounded-md px-4 py-3 flex items-center justify-between">
      ...
      <input type="text" placeholder="搜索..." class="px-3 py-1.5 text-sm border border-gray-300 rounded w-64">
    </div>
    
    <!-- 内容画布 -->
    <div class="flex-1 bg-white rounded-md p-6 overflow-y-auto">
      <!-- 表格/图表/内容... -->
    </div>
  </div>
</div>
...
##### Layout-D: 网格仪表盘布局 (Grid Dashboard)
**结构**: 多列网格布局
...
<div class="flex-1 bg-[#F5F6F8] p-3 overflow-y-auto">
  ...
  <div class="grid grid-cols-2 gap-3">
    <!-- 卡片 1 -->
    <div class="bg-white rounded-md p-6" data-type="global">
      <h3 class="text-sm font-medium text-gray-700 mb-4">统计卡片 1</h3>
      <div class="text-2xl font-bold text-gray-900">1,234</div>
    </div>
    
    <!-- 卡片 2 -->
    <div class="bg-white rounded-md p-6" data-type="global">
      <h3 class="text-sm font-medium text-gray-700 mb-4">统计卡片 2</h3>
      <div class="text-2xl font-bold text-[#087F9C]">56</div>
    </div>
    
    <!-- 卡片 3 (跨 2 列) -->
    <div class="col-span-2 bg-white rounded-md p-6" data-type="global">
      <h3 class="text-sm font-medium text-gray-700 mb-4">趋势图表</h3>
      <!-- 图表内容... -->
    </div>
  </div>
  ...
  <div class="grid grid-cols-3 gap-3 mt-3">
    <div class="bg-white rounded-md p-6" data-type="global">卡片 A</div>
    <div class="bg-white rounded-md p-6" data-type="global">卡片 B</div>
    <div class="bg-white rounded-md p-6" data-type="global">卡片 C</div>
  </div>
</div>
```
...
##### 布局选择指南 (Selection Guide)

| 布局类型 | 主要特征 | 推荐场景 | Tailwind 核心类 |
| :--- | :--- | :--- | :--- |
| **Layout-A** | 左右分栏 (1:2) | 配置+预览 | `grid grid-cols-3 gap-3` |
| **Layout-B** | 单列堆叠 | 表单/详情 | `flex flex-col gap-3` |
| **Layout-C** | 工具栏+内容 | 表格/图表 | `flex flex-col gap-3` + 固定顶栏 |
| **Layout-D** | 多列网格 | 仪表盘/概览 | `grid grid-cols-2 gap-3` |
