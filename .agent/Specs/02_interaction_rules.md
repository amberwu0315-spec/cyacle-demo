...
**用途**: 
- 树形列表中的单个节点，承载血统权限与业务状态。

**交互触发器 (Triggers)**:
| 动作 | 触发行为 | 逻辑 |
| :--- | :--- | :--- |
| **单击整行** | `onSelect` | 选中高亮，右侧详情面板同步加载。 |
| **双击整行** | `onRename` | **由于血统权限限制**：仅限 `origin: 'self'` 节点可原位激活文本编辑。 |
| **点击箭头** | `onToggle` | 仅展开/折叠，必须 `stopPropagation` 严禁触发选中。 |
| **悬停 (Hover)** | `showQuickActions` | 在节点最右侧浮现 `[+]` (添加) 和 `[...]` (更多) 快捷按钮。 |
| **右键点击** | `openContextMenu` | **根据权限矩阵渲染**：继承节点屏蔽“物理删除”、“重命名”、“移动”等选项。 |

**聚焦模式 (Focus View)**:
- **高亮逻辑**: 匹配搜索或手动聚焦的节点及其子级正常显示。
- **降级逻辑**: 聚焦节点的**所有上级路径节点**显示为面包屑灰 (`text-gray-300`)。
- **排除逻辑**: 已屏蔽 (`screened`) 的节点在大批量聚焦操作中默认跳过。
