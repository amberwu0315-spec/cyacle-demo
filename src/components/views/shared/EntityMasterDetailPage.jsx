...
export default function EntityMasterDetailPage({
    ...
    const collapseAction = onCollapse ? (
        <button
            type="button"
            onClick={onCollapse}
            className="inline-flex items-center gap-1 px-2 h-7 rounded border border-slate-200 text-xs text-slate-600 hover:text-[#0EA5B7] hover:border-[#0EA5B7] transition-colors shrink-0"
            title="收起并返回表格"
        >
    ...
    const resolvedDetailContent = renderDetailContent
        ? renderDetailContent({ selectedItem, listData, selectedId })
        : (
    ...
    return (
        <MasterDetailPageTemplate
            ...
            detailHeaderSlot={resolvedDetailHeader}
            detailContentSlot={resolvedDetailContent}
